...
every module's code on every interaction anywhere in the app -- is fixed by
switching to `st.navigation`, which only runs the active page (see the
top-level `scripts/README.md`).
