...
bigger, separate performance factor (whether `main.py`'s tab-based
navigation re-renders every module on every interaction) is tracked in the
local `TASKS.md`, not fixed here.
